...
def App(args):
    ...
    main App 
    """
    global done, points, current, prev_current, frame, homography_type, check_homography

    if args.select_mode == "classify_homography":
    ...
    cv2.namedWindow("HW4")
    cv2.setMouseCallback("HW4", on_mouse)
    ...
    while True:
        ...
        cv2.imshow("HW4", draw_frame)
        if cv2.waitKey(50) == 27:
            print("Escape hit, closing...")
            break

    cv2.destroyWindow("HW4")

if __name__ == '__main__':

    # select option
    parser = argparse.ArgumentParser(description='HW4 implementation')
    parser.add_argument('--select_mode',
        choices=["get_polygon_area", "classify_homography"],
        help='Select mode : choices=["get_polygon_area", "classify_homography"]'
    )
